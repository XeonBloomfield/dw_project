...
if __name__ == "__main__":
    ...
    for replay in replays_raw:
        ...
        print("Loading replay: " + replay_db.filehash)
        ...
        for curr_fps in range(0, replay.frames, int(replay.game_fps)):
            print("\tLoading frames from " + str(curr_fps) +
                  " to " + str(curr_fps + int(replay.game_fps)))
            start_time = time.time()
            # GameObjects
            for game_object in replay.objects.values():
                if game_object.id == 0:
                    continue
                if curr_fps <= game_object.started_at < curr_fps + int(replay.game_fps):
            ...
            for game_object in replay.objects.values():
                if game_object.died_at is None:
                    continue
                if curr_fps <= game_object.died_at < curr_fps + int(replay.game_fps):
            ...
            for tracker_event in replay.tracker_events:
                if curr_fps <= tracker_event.frame < curr_fps + int(replay.game_fps):
                    # PlayerStatsEvent
                    ...
                    # TrackerEvent
                        ...
                        tracker_event_game_objects.append(
                            tracker_event_game_object_db)

            # GameEvents
            for game_event in replay.game_events:
                if curr_fps <= game_event.frame < curr_fps + int(replay.game_fps):
                    ...
                    if hasattr(game_event, "objects"):
                        for obj in game_event.objects:
                            ...
                            game_event_game_objects.append(
                                game_event_game_object_db)
            ...
            print("\t\tDifference is: " + str(difference) + " seconds")
            if(1 - difference > 0):
                print("\t\tSleeping: " + str(1 - difference) + " seconds")
